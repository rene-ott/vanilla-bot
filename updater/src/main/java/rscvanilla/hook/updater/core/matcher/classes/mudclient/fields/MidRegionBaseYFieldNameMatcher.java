...
@YamlClassMemberKey("mid_region_base_y")
public class MidRegionBaseYFieldNameMatcher extends ClassMemberNameBaseMatcher implements MudClientClassFieldNameMatcher {

    public MidRegionBaseYFieldNameMatcher() {
        pattern = Pattern.compile(
            "(?<pre> {3}public int [a-zA-Z]{2} = 0;\\R" +
            " {3}public int [a-zA-Z]{2};\\R" +
            " {3}public int )(?<fieldName>[a-zA-Z]{2})(?<post>;\\R" +
            " {3}public com\\.rsc\\.b\\.e [a-zA-Z]{2};)",
            Pattern.MULTILINE
        );
    }
}
