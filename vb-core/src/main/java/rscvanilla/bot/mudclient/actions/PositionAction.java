...
public class PositionAction extends BaseAction {
    ...
    public boolean isPositionInDistance(Position tileToPos, int distance) {
        return mudClientWrapper.getLocalPlayer().getGlobalPosition().distanceTo(tileToPos) <= distance;
    }

    public boolean isPositionInDistance(int x, int y, int distance) {
        return mudClientWrapper.getLocalPlayer().getGlobalPosition().distanceTo(new Position(x, y)) <= distance;
    }
}
